import { Product } from "../../models/product";

export const multiProductHardcodedOne: Product[] = [
  {
    productId: '3dbff070-572b-4a21-8c73-95666d8b3cb7',
    productName: 'BEAUTY OF JOSEON - Relief Sun : Rice + Probiotic SPF50+ PA++++ - 10ml',
    // https://www.stylevana.com/en_US/beauty-of-joseon-relief-sun-rice-probiotic-50ml.html
    encodedProductUrl: 'https%253A%252F%252Fwww.stylevana.com%252Fen_US%252Fbeauty-of-joseon-relief-sun-rice-probiotic-50ml.html',
    createdDate: new Date('1/20/1970'),
    deletedDate: null,
    imagePath: 'https://cdn.stylevana.com/media/catalog/product/cache/1/image/9df78eab33525d08d6e5fb8d27136e95/b/e/beauty-of-joseon-relief-sun-rice-probiotic-50ml-318.jpg',
    options: {
      Size: '10ml',
      Color: 'N',
    },
    priceHistory: [
      { Price: 3.29, UpdatedDate: new Date('1/20/1970') },
      { Price: 3.69, UpdatedDate: new Date('11/21/2023') },
      { Price: 2.89, UpdatedDate: new Date('11/29/2023') },
      { Price: 3.29, UpdatedDate: new Date('12/15/2023') },
    ],
    newestPrice: 3.29,
    webdomain: 'Stylevana'

  },
  {
    productId: '3dbff070-572b-4a21-8c73-95351a3brdadsf7',
    productName: 'BEAUTY OF JOSEON - Relief Sun : Rice + Probiotic SPF50+ PA++++ - 50ml',
    encodedProductUrl: 'https%253A%252F%252Fwww.stylevana.com%252Fen_US%252Fbeauty-of-joseon-relief-sun-rice-probiotic-50ml.html',
    createdDate: new Date('1/20/1970'),
    deletedDate: null,
    imagePath: 'https://cdn.stylevana.com/media/catalog/product/cache/1/image/9df78eab33525d08d6e5fb8d27136e95/b/e/beauty-of-joseon-relief-sun-rice-probiotic-50ml-318.jpg',
    options: {
      Size: '50ml',
      Color: 'N',
    },
    priceHistory: [
      { Price: 13.29, UpdatedDate: new Date('1/20/1970') },
      { Price: 13.69, UpdatedDate: new Date('11/21/2023') },
      { Price: 12.89, UpdatedDate: new Date('11/29/2023') },
      { Price: 13.29, UpdatedDate: new Date('12/15/2023') },
    ],
    newestPrice: 13.29,
    webdomain: 'Stylevana'
  },
];
